...
public class DateYMD extends Date {
    ...
    public void Increment(int increment) {
        for (int count = 1; count <= increment; count++) {
            if (getDay() + 1 > getDay(getMonth(), getYear()) && getMonth() == 12) {
                setYear(getYear() + 1);
                setMonth(1);
                setDay(1);
                count++;
            } else if (getMonth() != 12 && getDay() + 1 > getDay(getMonth(), getYear())) {
                setDay(1);
                setMonth(getMonth()+1);
                count++;
            } else
    ...
    public void Decrement(int decrement) {
        for (int count = 1; count <= decrement; count++) {
            if (getDay() - 1 <= 0 && getMonth() == 1) {
                setYear(getYear()-1);
                setMonth(12);
                setDay(getDay(getMonth(), getYear()));
                count++;
            } else if (getMonth() != 1 && getDay() - 1 <= 0) {
                setDay(getDay(getMonth() - 1, getYear()));
                setMonth(getMonth()-1);
                count++;
            } else
                setDay(getDay()-1);
